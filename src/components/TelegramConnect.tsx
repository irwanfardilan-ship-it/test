...
export function TelegramConnect() {
  ...
  const botUsername = appConfig.BOT_USERNAME || import.meta.env.VITE_BOT_USERNAME || 'azurlize_recruitment_bot';
      ...
      >
        ...
        <div className="mt-6 p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/20 text-xs text-rose-700 dark:text-rose-300 text-left space-y-2">
          ...
          <p className="leading-relaxed text-[11px] opacity-90">
            {telegramAuthError || 'Aplikasi hanya dapat diakses melalui Bot resmi.'}
          </p>
        </div>
        ...
        {tgUser && (
          <div className="mt-6 p-4 rounded-2xl bg-slate-100/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-left">
            <p className="text-[10px] font-bold tracking-wider text-slate-500 uppercase mb-3">Sistem Mendeteksi Anda Sebagai:</p>
            <div className="space-y-1.5 text-xs font-mono text-slate-600 dark:text-slate-400">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{tgUser.id}</span>
              </div>
            </div>
            <button
              onClick={copyDetails}
              className="mt-4 w-full py-2 px-3 bg-white dark:bg-white/10 hover:bg-slate-100 dark:hover:bg-white/20 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-semibold transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
              {copied ? (
                <>
                ...
                <>
                  ...
                  <span>Salin Identitas & Laporkan Bantuan</span>
                </>
              )}
            </button>
          </div>
        ...
        <div className="mt-6 pt-5 border-t border-slate-200 dark:border-white/5 flex items-center justify-center gap-1.5 text-[9px] font-mono text-slate-400 dark:text-slate-500">
          <span>UNAUTHORIZED_ACCESS_BLOCKED</span>
        </div>
      </motion.div>
    </div>
  );
